// TextTag.h - Declares the TextTag class.
//
...
#ifndef SPC_TEXT_TAG_H
#define SPC_TEXT_TAG_H
...
namespace Spc
{
    ...
    /// use a mixture of text and binary numbers. That said, text and binary 
    /// tags have slightly different offsets, and dumps that mix text and binary 
    /// still tend to use the offsets of one format or the other.
    struct TextTag : public DataStructure
    {
        ...
        {
            "Song Length (sec)", 0xA9, 3, NumericType::Text
        };

        /// @brief The length of fade out, in milliseconds.
        NumericField fadeLength
        { 
            "Fade Length (ms)", 0xAC, 5, NumericType::Text 
        };

        /// @brief The composer of the song.
        TextField songArtist{ "Song Artist", 0xB1, 32 };
