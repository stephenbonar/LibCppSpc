...
namespace Spc
{
    ...
    public:
        ...
        Spc::BinaryTag BinaryTag() const { return binaryTag; }
    ...
    private:
        ...
        T GetField(ExtendedTagItem* item, int id, int defaultSize) const
        {
            if (item != nullptr)
            {
                // When the value is stored in the item's data field (in the 
                // item's header), we have to create a new field with the 
                // correct label  and copy the data over to it.
                ...
                // In contrast, if the value is stored beyond the header, it 
                // will have already been initialized with the proper label and
                // we simply need to cast it to the correct type.
                if (item->extendedData != nullptr)
                {
                    Field* field = item->extendedData.get();
                    T* convertedField = dynamic_cast<T*>(field);
                    return *convertedField;
                }
            }

            // If we reach this point, no existing data was found so we
            // initalize an empty field with a default size.
                            ...
                            std::shared_ptr<ExtendedTagItem>& item)
        {
            ...
            if (cmd.extendedType == extendedTypeDataInHeader)
            {
                ...
                {
                    auto d = InitExtendedField<TrackField>(cmd.extendedID, 2);
                    d->SetValue(cmd.value);
                    item->data = d;
                }
                else if (cmd.extendedID == extendedMutedVoicesID)
                {
                    auto d = InitExtendedField<BinaryField>(cmd.extendedID, 2);
                    d->SetValue(cmd.value);
                    item->data = d;
                }
                else
                {
                    auto d = InitExtendedField<NumericField>(cmd.extendedID, 2);
                    d->SetType(NumericType::Binary);
                    d->SetValue(cmd.value);
                    item->data = d;
                }
            }
            ...
            {
                // Convert the data field pointer to a numeric field (assumes 
                // the data is not stored in the extended area).
                auto size = std::static_pointer_cast<NumericField>(item->data);
                size->SetType(NumericType::Binary);

                if (cmd.extendedType == extendedTypeString)
                {
                    auto field = 
                        InitExtendedField<TextField>(cmd.extendedID, 
                                                     cmd.value.size());

                    // Set the size / data to the size of the value as they 
                    // should match.
                ...
                {
                    auto field =
                        InitExtendedField<NumericField>(cmd.extendedID, 4);
                    size->SetValue(4);
